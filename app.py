...
# 导出嵌入图片的目录（每次根据文件 hash/mtime 建目录更稳；这里用 mtime+size 简化）
EXPORT_ROOT = "/mnt/data/_extracted_images"  # 沙盒可写
...
def _kv_html(columns: List[str], row: dict, exclude_cols: List[str] = []) -> str:
    ...
    for c in columns:
        ...
        rows_html.append(f'<tr><td class="k">{_html.escape(str(c))}</td><td>{v_html if v_html else "&nbsp;"}</td></tr>')
